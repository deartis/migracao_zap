...
function getClientForUser(token) {
    if (!clients.has(token)) {
        ...
        client.on('qr', qr => {
            ...
            console.log(`Novo QR Code gerado para token: ${token.substring(0, 10)}...`);
        ...
        // Evento de pronto
        client.on('ready', () => {
            console.log(`Cliente WhatsApp pronto para token: ${token.substring(0, 10)}...`);
        ...
        client.on('disconnected', (reason) => {
            console.log(`Cliente desconectado para token ${token.substring(0, 10)}...: ${reason}`);
...
app.get('/delete-session', extractToken, async (req, res) => {
    try {
        ...
        if (clientData) {
            ...
            const sessionDir = path.join(__dirname, `.wwebjs_auth/session-user-${token}`);
    ...
    try {
        ...
        if (media) {
            try {
                ...
                const sent = await clientData.client.sendMessage(chatId, mediaMessage, { caption: message });
