    ...
    public function extractContacts(array $dataArrays)
    {
        ...
        foreach ($dataArrays as $row) {
            ...
            if (!isset($row[$nameField])) {
                // Se faltar o campo de nome, ignora (não conta como erro)
                continue;
            }

            $name = $this->formatCell($row[$nameField]);

            // Verificar se o telefone existe e é válido
            if (!isset($row[$phoneField]) || $row[$phoneField] === null) {
                // Telefone não existe ou é explicitamente NULL
                $contacts[] = [
                    'name' => $name,
                    'phone' => null,
                    'type_error' => 'invalid_number',
                    'metadata' => $row
                ];
                $errorCount++;
            } else {
                ...
                if (empty($phone)) {
                    // Telefone existe mas não é válido após sanitização
                    $contacts[] = [
                        'name' => $name,
                        'phone' => null,
                        'type_error' => 'invalid_number',
        ...
        return [
            ...
            'message' => count($contacts) . ' contatos encontrados. ' . $errorCount . ' telefones inválidos.',
            'errors' => $errorCount,
            'contacts' => $contacts,
            'nameField' => $nameField,
