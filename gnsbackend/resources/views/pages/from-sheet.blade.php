...
{{--
@extends('layout.app') --}}
{{-- Ou o nome do seu layout base --}}{{--


@section('content')
    <!-- 1️⃣ INPUT DE UPLOAD XLSX -->
    <div id="importacaoXlsx">
        <label for="fileInput">Importar Arquivo Excel:</label>
        <input type="file" id="fileInput" accept=".xlsx, .xls" />
    </div>

    <!-- 2️⃣ SELEÇÃO DE COLUNAS (INICIALMENTE ESCONDIDO) -->
    <div id="selecaoColunas" style="display:none;">
        <h4>Selecione as colunas para placeholders</h4>
        <form id="formSelecaoColunas"></form>
        <button type="button" id="btnConfirmarSelecao" class="btn btn-primary mt-2">Confirmar</button>
    </div>

    <!-- 3️⃣ TEXTAREA E PLACEHOLDERS (INICIALMENTE ESCONDIDO) -->
    <div id="editorMensagem" style="display:none;">
        <h4>Mensagens em Massa:</h4>
        <textarea id="messageTextarea" class="form-control" rows="8"></textarea>

        <h5 class="mt-3">Colunas da Tabela</h5>
        <div id="botoesPlaceholders" class="mb-3"></div>

        <button type="button" class="btn btn-success">Enviar Mensagens</button>
    </div>
    <script>
        document.getElementById('fileInput').addEventListener('change', function(e) {
            ...
            reader.onload = function(event) {
                ...
                if (jsonData.length > 0) {
                    const colunas = jsonData[0]; // Primeira linha são os headers

                    // Oculta input e exibe seleção de colunas
                    document.getElementById('importacaoXlsx').style.display = 'none';
                    document.getElementById('selecaoColunas').style.display = 'block';

                    renderizaSelects(colunas);
                }
            };
            reader.readAsArrayBuffer(file);
        });

        function renderizaSelects(colunas) {
            const form = document.getElementById('formSelecaoColunas');
            ...
            colunas.forEach(coluna => {
                ...
                label.innerText = coluna;
                label.classList.add('form-label', 'fw-bold', 'd-block');

                const select = document.createElement('select');
                select.classList.add('form-select', 'coluna-select');
                select.dataset.coluna = coluna;

                const optionNaoUsar = new Option('---Não usar---', '');
                select.appendChild(optionNaoUsar);

                const optionUsar = new Option('Usar como placeholder', coluna);
                select.appendChild(optionUsar);
        ...
        document.getElementById('btnConfirmarSelecao').addEventListener('click', function() {
            const selects = document.querySelectorAll('.coluna-select');
            const colunasSelecionadas = [];

            selects.forEach(select => {
                if (select.value) {
                    colunasSelecionadas.push(select.value);
                }
            });
            ...
            // Oculta seleção e exibe editor de mensagem
            document.getElementById('selecaoColunas').style.display = 'none';
            document.getElementById('editorMensagem').style.display = 'block';

            renderizaBotoesPlaceholders(colunasSelecionadas);
        });

        function renderizaBotoesPlaceholders(colunas) {
            const container = document.getElementById('botoesPlaceholders');
            container.innerHTML = '';
            ...
            colunas.forEach(coluna => {
                ...
                button.type = 'button';
                button.classList.add('btn', 'btn-info', 'btn-sm', 'm-1');
                button.dataset.placeholder = coluna;
                button.innerText = coluna;
